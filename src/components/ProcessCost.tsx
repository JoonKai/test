  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-gray-800">공정별 웨이퍼당 원가 비교</h3>
          <p className="text-xs text-gray-400 mt-0.5">설정 변경 시 실시간 업데이트</p>
        </div>
        <div className="text-right">
          <div className="text-xs text-gray-400">4공정 합계</div>
          <div className="text-lg font-bold text-gray-800 font-mono">{formatKRW(total)} 원/매</div>
        </div>
      </div>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <BarChart data={data} layout="vertical" margin={{ left: 0, right: 30, top: 0, bottom: 0 }}>
          ...
          <XAxis
            ...
            tickLine={false}
          />
          <YAxis type="category" dataKey="name" width={42} tick={{ fontSize: 12, fontWeight: 600 }} axisLine={false} tickLine={false} />
          <Tooltip
            {...CHART_TOOLTIP_STYLE}
  ...
  return (
    <div className="sticky top-4 bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-4">
      <div>
        ...
        <div className="text-xl font-bold text-gray-800 font-mono mt-0.5">{highlight}</div>
      </div>
      {children}
    </div>
  );
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} layout="vertical" margin={{ left: 0, right: 30, top: 0, bottom: 0 }}>
        ...
        />
        <YAxis type="category" dataKey="name" width={52} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
        <Tooltip
          {...CHART_TOOLTIP_STYLE}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1">
          <StickyChartPanel title="실시간 원가 분석" highlight={`${formatKRW(totalPerWafer)} 원/매`}>
            <div>
              <p className="text-xs text-gray-400 mb-2 font-medium">런당 원가 구성</p>
              <CostBarChart data={chartData} height={180} />
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-2 font-medium">원가 비율</p>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie
                  ...
                  <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(v: number) => `${formatKRW(v)} 원`} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100">
              <div className="text-center">
                <div className="text-xs text-gray-400">런당 합계</div>
                <div className="text-sm font-bold font-mono text-gray-800">{formatKRW(totalPerRun)}</div>
              </div>
              <div className="text-center">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1">
          <StickyChartPanel title="실시간 원가 분석" highlight={`${formatKRW(totalPerWafer)} 원/매`}>
            <div>
              <p className="text-xs text-gray-400 mb-2 font-medium">원가 구성</p>
              <CostBarChart data={costData} height={150} />
            </div>
            <div>
              <p className="text-xs text-gray-400 mb-2 font-medium">시간 구성 (초)</p>
              ...
              <div className="flex gap-2 flex-wrap mt-1">
                {timeData.map((d) => (
                  <div key={d.name} className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-gray-500">{d.name} {Math.round(d.value / 60)}분</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="xl:col-span-1">
          <StickyChartPanel title="실시간 원가 분석" highlight={`${formatKRW(totalPerWafer)} 원/매`}>
            {itemChartData.length > 0 ? (
              <>
                <div>
                  <p className="text-xs text-gray-400 mb-2 font-medium">항목별 런당 원가</p>
                  <ResponsiveContainer width="100%" height={Math.max(120, itemChartData.length * 36)}>
                    <BarChart data={itemChartData} layout="vertical" margin={{ left: 0, right: 30 }}>
                      <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f0f0f0" />
                      <XAxis type="number" tickFormatter={(v) => v >= 1000 ? `${(v / 1000).toFixed(0)}K` : v} tick={{ fontSize: 10 }} tickLine={false} />
                ...
                <div className="pt-2 border-t border-gray-100 space-y-1">
                  {itemChartData.map((d) => (
                    <div key={d.name} className="flex justify-between text-xs">
                      ...
                      <span className="font-mono font-medium text-gray-700">{formatKRW(d.total)}원</span>
                    </div>
                  ))}
                </div>
              </>
            ) : (
              <div className="text-xs text-gray-400 text-center py-8">측정 항목을 추가하면 차트가 표시됩니다.</div>
            )}
            <div className="pt-2 border-t border-gray-100 text-center">
              <div className="text-xs text-gray-400">{measurements.length}개 항목 · 런당 합계</div>
              <div className="text-sm font-bold font-mono text-gray-800">{formatKRW(totalPerRun)}</div>
            </div>
          </StickyChartPanel>
        </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1">
          <StickyChartPanel title="실시간 원가 분석" highlight={`${formatKRW(totalPerWafer)} 원/매`}>
            <div>
              <p className="text-xs text-gray-400 mb-2 font-medium">원가 구성 (런당)</p>
              <CostBarChart data={costData} height={140} />
            </div>
            {matBreakdown.length > 0 && (
              <div>
                <p className="text-xs text-gray-400 mb-2 font-medium">재료비 상세</p>
